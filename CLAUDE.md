...
```json
{
  ...
  "steps": [
    ...
    {"action": "assert", "assertion": {"type": "exists", "selector": ".dashboard"}}
  ]
}
```

Step types (generated from `src/test-actions.ts`):

<!-- GENERATED:step-actions -->
<!-- Do not edit by hand — see src/test-actions.ts -->
| action | fields | what it does | example |
| --- | --- | --- | --- |
| `navigate` | `url` | Load a URL, then wait for the widget to reconnect in that same tab. | `{"action": "navigate", "url": "http://localhost:3000/login"}` |
| `click` | `selector` or `ref` | Realistic click (scroll into view, mouseenter/over/down/up/click). Prefers the match a user could act on when several exist. | `{"action": "click", "selector": "button:text(Sign in)"}` |
| `type` | `selector`/`ref`, `text`, `clear`, `paste`, `humanlike` | Per-character typing with the full key lifecycle. `paste: true` for fast entry that still triggers framework validation. | `{"action": "type", "selector": "#email", "text": "a@b.com"}` |
| `check` | `selector` or `ref` | Toggle a checkbox or radio via a realistic click. | `{"action": "check", "selector": "#agree"}` |
| `key` | `key`, `selector`, `modifiers` | Press a key, optionally with modifiers, at the focused element. | `{"action": "key", "key": "Enter"}` |
| `select-text` | `selector`, `text` | Select TEXT within an element (generated by the recorder) — not choosing a dropdown option. | `{"action": "select-text", "selector": "#note"}` |
| `cut` | `selector`, `text` | Dispatch a cut event at the element (generated by the recorder). | `{"action": "cut", "selector": "#note"}` |
| `copy` | `selector`, `text` | Dispatch a copy event at the element (generated by the recorder). | `{"action": "copy", "selector": "#note"}` |
| `paste` | `selector`, `text` | Dispatch a paste event at the element (generated by the recorder). | `{"action": "paste", "selector": "#note"}` |
| `drag` | `selector`/`ref`, `deltaX`, `deltaY`, `duration` | Drag from the element's centre by a pixel delta — sliders, resize handles, drag-reorder lists. | `{"action": "drag", "selector": ".slider-thumb", "deltaX": 50}` |
| `wait` | `duration` (ms), `selector` (alias `forElement`), `forWindow`, `url` | Pause, or poll until something is true. **Must be given something to wait for** — a wait with none of these fields is an error, not a no-op. | `{"action": "wait", "selector": ".dashboard", "timeout": 10000}` |
| `assert` | `assertion` — `exists`, `visible`, `hidden`, `text`, `value`, `attribute`, `url` | Fail the step unless the assertion holds. | `{"action": "assert", "assertion": {"type": "visible", "selector": ".dashboard"}}` |
| `eval` | `code` | Run JavaScript in the page. Async works; multi-statement code needs an explicit `return`. | `{"action": "eval", "code": "return document.title"}` |
| `verify` | `eval`, `expect` | Poll an expression until it matches an expectation (or time out). | `{"action": "verify", "eval": "app.items.length", "expect": {"equals": 3}}` |
| `tabs-open` | `url` | Open a new tab (desktop app). Follow with `wait` + `forWindow` to await its widget. | `{"action": "tabs-open", "url": "http://localhost:3000"}` |
| `tabs-close` | `window` | Close a tab by window id. | `{"action": "tabs-close", "window": "hj-abc-1"}` |
| `tabs-focus` | `window` | Point untargeted commands at a tab — a server-side routing change, so it cannot time out on a hidden tab. | `{"action": "tabs-focus", "window": "hj-abc-1"}` |
<!-- END:step-actions -->

**The canonical list lives in `src/test-actions.ts` (`TEST_STEP_ACTIONS`), and
`src/test-actions.test.ts` asserts it matches the runner's `switch (step.action)` in both
directions.** An endpoint existing does NOT make it a step: `/drag` shipped long before the
runner had a `drag` case, so a documented capability failed with "Unsupported step action"
while `hj test validate` passed the file clean. Add a step type to the list, the union in
`types.ts`, and the runner together — the test fails if you don't.

`select-text`/`cut`/`copy`/`paste` are generated by the recorder and replayed as the corresponding DOM event dispatched at the step's `selector` (`select-text` dispatches the DOM `select` event — the rename is ours, the platform's event name is not). `select` remains a deprecated alias. The runner fails on any unrecognized action (no silent pass).
...
15. **Marker-delimited regions INSIDE three hand-written docs** — `CLAUDE.md`, `SKILL.md` and
    `docs/CI-INTEGRATION.md` each contain `<!-- GENERATED:step-actions --> … <!-- END -->` blocks
    rewritten from `src/test-actions.ts` on every build. Every other rule here names whole FILES, so
    those three files' reputation says "hand-written" and an edit inside a block gets silently
    reverted by the next build. Edit `src/test-actions.ts` instead. A named target whose marker is
    missing is now a build FAILURE rather than a silent skip — a block nobody writes stays clean, so
    docs-drift would never have caught it.
...
non-TCP channel. That cost WAS paid — see the stdio channel below, which restored the tabs. If you
are tempted to re-open one of these routes "just for the app", note that a drive-by page reaches a
desktop-app server exactly as easily.
