...
3. **To find out what's installed, ask it.** `installedVersion()` prefers `~/.haltija/hj-install.json`, and falls back to running `hj --version` (which exists as of 1.4.0). A copy that can't answer is a pre-1.4.0 one — exactly the stale artifact we want to replace. Same principle as retiring a server by asking rather than guessing at a pid: the artifact can tell you what it is, so don't infer it.
