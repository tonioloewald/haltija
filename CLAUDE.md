...
| --- | --- | --- |
| `navigate` | `url` | Load a URL, then wait for the widget to reconnect in that same tab. |
| `click` | `selector` or `ref` | Realistic click (scroll into view, mouseenter/over/down/up/click). Prefers the match a user could act on when several exist. |
| `type` | `selector`/`ref`, `text`, `clear`, `paste`, `humanlike` | Per-character typing with the full key lifecycle. `paste: true` for fast entry that still triggers framework validation. |
| `check` | `selector` or `ref` | Toggle a checkbox or radio via a realistic click. |
| `key` | `key`, `selector`, `modifiers` | Press a key, optionally with modifiers, at the focused element. |
| `select-text` | `selector`, `text` | Select TEXT within an element (generated by the recorder) — not choosing a dropdown option. |
| `cut` | `selector`, `text` | Dispatch a cut event at the element (generated by the recorder). |
| `copy` | `selector`, `text` | Dispatch a copy event at the element (generated by the recorder). |
| `paste` | `selector`, `text` | Dispatch a paste event at the element (generated by the recorder). |
| `drag` | `selector`/`ref`, `deltaX`, `deltaY`, `duration` | Drag from the element's centre by a pixel delta — sliders, resize handles, drag-reorder lists. |
| `wait` | `duration` (ms), `selector` (alias `forElement`), `forWindow`, `url` | Pause, or poll until something is true. **Must be given something to wait for** — a wait with none of these fields is an error, not a no-op. |
| `assert` | `assertion` — `exists`, `visible`, `hidden`, `text`, `value`, `attribute`, `url` | Fail the step unless the assertion holds. |
| `eval` | `code` | Run JavaScript in the page. Async works; multi-statement code needs an explicit `return`. |
| `verify` | `eval`, `expect` | Poll an expression until it matches an expectation (or time out). |
| `tabs-open` | `url` | Open a new tab (desktop app). Follow with `wait` + `forWindow` to await its widget. |
| `tabs-close` | `window` | Close a tab by window id. |
| `tabs-focus` | `window` | Point untargeted commands at a tab — a server-side routing change, so it cannot time out on a hidden tab. |
<!-- END:step-actions -->
