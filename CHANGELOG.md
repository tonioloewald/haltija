...
- **Only the server holding 8700 takes the well-known HTTPS port 8701.** A server started with
  `--port`/`--name`, or one that fell back off a busy 8700 onto an ephemeral port, now opens HTTPS
  on an ephemeral port shown in its banner and `/status`, unless given `--https-port`. Before
  this, with `both` as the default, the first 1.13 server started in any project could capture
  every project's https pages while the real channel and `hj` saw 0 tabs.

- **The startup banner and the `server` export prefer HTTP.** They preferred HTTPS, which with
  `both` as the default pointed `curl` at a self-signed certificate and made the DEV MODE
  one-liner fail silently on http pages. `server` is the HTTP listener whenever HTTP is up; use
  `httpsServer` for the HTTPS one.

- **A server run from a source checkout installs `hj` by copy, never by symlink,** and only a
  bundle whose stamped version matches the server's. The old symlink fallback pinned
  `~/.local/bin/hj` to that checkout permanently, because a symlinked `hj` is treated as a
  deliberate choice and never repaired.
...
- **The machine-level certificate is renewed before it expires**, and one that can't be read is
  replaced. It used to be regenerated by every `npm install`; kept in `~/.haltija/certs` it would
  have lapsed after the openssl fallback's 365 days, while `/status` still reported HTTPS up.
  Writing, adopting or renewing it now leaves a receipt in `~/.haltija/machine-actions.log` and a
  line on stderr. A replaced pair is kept as `*.bak`, since `HALTIJA_CERTS_DIR` may point at a
  directory you own. A key that doesn't match its cert is replaced too, so a pair left
  mismatched by two servers renewing at once heals on the next start. A failed renewal keeps
  using the current cert while it is still valid.

- **A bad certificate costs HTTPS only.** A mismatched or corrupt pair in `~/.haltija/certs` used
  to stop every haltija server on the machine from starting, HTTP included. `/status` (and so
  `hj where`) now names the real reason HTTPS is down; it used to blame "port N is held by
  another process" for certificate problems.

- **`testInBrowser`: `clickAt` and `dragFrom` send pointer events** as well as mouse events, with
  `buttons` set, and moves go to the element under the pointer. Pointer-driven controls (3D camera
  orbits) ignored them before, and the drag still reported success. `clickAt` also accepts a real
  point above the fold on a scrolled page, which it rejected as "outside the document", and
  `click`/`type`/`press` now fail on a `{ success: false }` reply from a bridge instead of passing.

- **The test suites no longer touch the real 8701 or `~/.haltija/certs`.** The Playwright helper
  opted out of HTTPS with a variable nothing had read since v0.1.7, which stopped mattering only
  while `http` was the default. CI now fails if a suite writes anything under `~/.haltija`.

- **The README no longer says "localhost only by default".** The server listens on all
  interfaces, and without `--token` any page or anything on the network can call its REST API.
