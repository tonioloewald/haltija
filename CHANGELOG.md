...
- **A server run from a source checkout installs `hj` by copy, never by symlink.** The old
  symlink fallback pinned `~/.local/bin/hj` to that checkout permanently, because a symlinked `hj`
  is treated as a deliberate choice and never repaired.
...
- **The machine-level certificate is renewed before it expires**, and one that can't be read is
  ...
  line on stderr, and it is written atomically.

- **A bad certificate costs HTTPS only.** A mismatched or corrupt pair in `~/.haltija/certs` used
  to stop every haltija server on the machine from starting, HTTP included.
