...
- [x] **The step runner has ZERO executable coverage.** No unit or Playwright lane calls `/test/run`
...
- [x] **`tests/haltija.test.ts` skip-guards to green.** `skipUnlessServer()` returns early and bun
...
- [x] `src/api-schema.ts:2000` holds a FOURTH step list (8 of 17), propagating to `API.md`,
  `apps/mcp/src/endpoints.json` and `embedded-assets.ts`. Interpolate `stepActionsInline()`.
- [x] `CLAUDE.md` test-JSON example uses the flat `{"action":"assert","type":"exists"}` shape the
...
- [x] `CLAUDE.md` prose still says "`select`/`cut`/`copy`/`paste` are generated by the recorder",
  eight lines below the generated table saying `select-text`.
- [x] `docs/CI-INTEGRATION.md` has an orphaned `### Step Types` heading and a bare table header row
...
- [x] `apps/mcp/build/endpoints.json` was **7 months stale** — re-checked 2026-08-30: it is now
  byte-identical to `apps/mcp/src/endpoints.json`, 67 endpoints, both committed 2026-08-28. The
  staleness is gone; only the `files` question below remains.
...
- [x] **`read` first — it is safe and it is most of the value.** Mirroring carries no injection risk,
  and seeing the agent work is the thing you cannot do today. Lets the page-side UI land against
  something real.
- [x] **`write` needs a decision first, and it is not the one the issue makes.** The proposal argues
...
- [x] Opt-in with its own name, never `haltijaDev: true` — a mirror carries **everything the agent
  prints**, including anything it echoes from a file it just read.
- [x] Remote input line-at-a-time, not character-at-a-time (two writers on one pty interleave badly).
...
- [ ] **Integration tiers can latch onto a transient `src/`-spawned server.** Bare `bun test`
      ...
      `bun test`.
- [x] Add the five new 1.4.0 modules to the CLAUDE.md Key Source Files table (`hj-install.ts`,
      `legacy-servers.ts`, `port-pid.ts`, `semver.ts`, `machine-log.ts`).
- [x] File the ecosystem/upstream items in `UPSTREAM.md` (which doesn't exist yet): `bun build
