...
function generateDocsMd(): string {
  ...
  lines.push('')
  lines.push('Each project should run its own haltija on a port of its choosing. Tell `hj`')
  lines.push('which one to talk to (per-shell):')
  lines.push('')
  lines.push('```bash')
  lines.push('export HALTIJA_PORT=9123        # all hj calls in this shell hit port 9123')
  lines.push('hj tree')
  lines.push('')
  lines.push('hj --port 9123 tree             # one-off port override')
  lines.push('```')
  lines.push('')
