...
function generateDocsMd(): string {
  ...
  lines.push('## Sessions')
  lines.push('')
  lines.push('Every Haltija widget runs in a **session** — a private namespace for your browser tabs.')
  lines.push('Widgets auto-generate a session token, or you can set one explicitly.')
  lines.push('')
  lines.push('To connect `hj` to your session, copy the token from the widget UI (click the')
  lines.push('token badge in the header) or set it:')
  lines.push('')
  lines.push('```bash')
  lines.push('export HALTIJA_SESSION=<token>        # Copy from widget UI')
  lines.push('hj tree                                # Only sees widgets in your session')
  lines.push('')
  lines.push('hj --session <token> tree              # One-off session override')
  lines.push('```')
  lines.push('')
  lines.push('Without a session, `hj` sees all widgets (backward compatible). In secure mode')
  lines.push('(`haltija --secure`), a session token is required.')
  lines.push('')
  lines.push('**For developers** — inject haltija into your app with an explicit session:')
  lines.push('')
  lines.push('```js')
  lines.push('import { inject } from \'haltija/component\'')
  lines.push('inject(\'ws://localhost:8700/ws/browser\', { session: \'my-debug-token\' })')
  lines.push('```')
  lines.push('')
  lines.push('Or via HTML:')
  lines.push('')
  lines.push('```html')
  lines.push('<haltija-dev server="ws://localhost:8700/ws/browser" session="my-debug-token"></haltija-dev>')
  lines.push('```')
  lines.push('')
