...
function generateDocsMd(): string {
  ...
  lines.push('')
  lines.push('## Multi-Agent Sessions')
  lines.push('')
  lines.push('When multiple agents share one Haltija server, each agent automatically gets')
  lines.push('its own **session**. Windows are claimed on first interaction — once an agent')
  lines.push('uses a tab, other agents won\'t accidentally target it.')
  lines.push('')
  lines.push('**How it works:**')
  lines.push('- `hj` generates a session ID per shell (stored in `$HALTIJA_SESSION`)')
  lines.push('- Every request sends `X-Haltija-Session` header automatically')
  lines.push('- First command that routes to a window claims it for that session')
  lines.push('- `hj windows` and `hj status` only show your own + unclaimed windows')
  lines.push('- Commands without a session header see all windows (backward compatible)')
  lines.push('')
  lines.push('```bash')
  lines.push('# Agent A claims a window by interacting with it')
  lines.push('hj tree                                    # Claims first available window')
  lines.push('')
  lines.push('# Agent B (different shell) gets its own window')
  lines.push('hj tree                                    # Claims a different unclaimed window')
  lines.push('')
  lines.push('# Explicit session control')
  lines.push('export HALTIJA_SESSION=my-session           # Set custom session ID')
  lines.push('hj windows                                  # See only your windows')
  lines.push('curl -X POST localhost:8700/windows/WIN_ID/unclaim  # Release a window')
  lines.push('```')
  lines.push('')
