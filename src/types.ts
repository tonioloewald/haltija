...
export interface DomQueryRequest {
  selector?: string       // CSS/text selector (omit when using ref)
  ref?: string            // Ref ID from /tree output (alternative to selector)
...
export interface KeyStep extends BaseStep {
  action: 'key'
  key: string
  /** Element to focus before dispatching the key (recorded for context); defaults to body */
  selector?: string
  modifiers?: { alt?: boolean, ctrl?: boolean, meta?: boolean, shift?: boolean }
}

/** Text-selection and clipboard steps generated by the recorder. */
export interface SelectStep extends BaseStep {
  /**
   * `select` is a DEPRECATED alias for `select-text`. It is kept so existing suites keep running,
   * and it is being freed: measured on agents, `select` unanimously reads as "choose an <option>",
   * which is a capability haltija does not have yet and which the word should eventually mean.
   */
  action: 'select-text' | 'select'
  selector: string
  /** The selected text (for description/verification) */
  text?: string
}

export interface ClipboardStep extends BaseStep {
  action: 'cut' | 'copy' | 'paste'
  selector: string
  /** The cut/copied/pasted text (for description/verification) */
  text?: string
}
...
export interface StepResult {
  /**
   * Advisory note about the step itself — currently a deprecated action spelling.
   *
   * Separate from `error` on purpose: the step RAN and PASSED. Folding a deprecation into `error`
   * would fail suites that are merely old, and folding it into nothing would mean authors discover
   * the rename when the alias is deleted.
   */
  warning?: string
...
export interface DomTreeNode {
  ...
  flags?: {
    ...
    hiddenReason?: 'display' | 'visibility' | 'hidden-attr' | 'aria-hidden' | 'opacity' | 'near-transparent' | 'pointer-events-none' | 'clipped' | 'zero-size' | 'off-screen' | 'collapsed-details'
