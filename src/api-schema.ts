...
export const console_ = endpoint({
  ...
  description: `Returns captured console.log/warn/error/info/debug from the page — AND uncaught
...
Response: { success, path?, image?, width, height, source, displaySurface?, canvas?, warning? }`,
  ...
  input: s.object({
    ...
    quality: s.number.describe('Quality for lossy formats (webp/jpeg). Either scale works: 0-1 (canvas-native) or 0-100 (percentage); anything above 1 is read as a percentage and clamped. Ignored for png.').optional,
...
pierces more) or \`hj map --image\` (which shows you the box that is there).`,
  ...
  input: s.object({
    ...
    image: s.boolean.describe('Also render the map as a schematic PNG (rasterized). NOTE: response.nodes is a TREE (recurse into .children — a <ul> holds its <li>s, a custom element holds its shadow content) while response.legend is a FLAT ref->facts index of only the ref-bearing nodes, so the two legitimately contain different sets. Vision cost scales with PIXELS, roughly (w*h)/750 for Claude — there is NO fixed floor, so a small or size-capped schematic can be very cheap (a 491x480 one is ~314 tokens; with maxWidth 200, ~52). ~1600 is the practical ceiling, since larger images are downscaled before tokenisation. response.cost reports approxJsonTokens and approxImageTokens for THIS page — compare those rather than assuming.').optional,
