...
export function resolveCertDir(
  ...
  // Spelled out, not `env[CERT_DIR_ENV]`: env-vars.test.ts checks every documented variable is
  // read somewhere by matching `env.NAME`, and the indirection hid this, the one real reader.
  return env.HALTIJA_CERTS_DIR || join(home, '.haltija', 'certs')
}
...
export type CertPlan =
  ...
  | { action: 'generate'; paths: CertPaths; replacing?: 'expiring' | 'unreadable' }

/** Renew this long before expiry, so a cert never lapses between two server starts. */
export const CERT_RENEW_WITHIN_MS = 30 * 24 * 60 * 60 * 1000
 ...
 *
 * EXPIRY. A per-install cert was regenerated by every `npm install`, so nothing ever had to check
 * a date. The machine-level one is kept forever, and the openssl fallback issues it for 365 days:
 * after a year HTTPS would keep "listening" while every https page failed the handshake — the
 * looks-healthy-isn't channel #32 was about (1.13.0-beta.1 review). So a cert expiring within
 * `CERT_RENEW_WITHIN_MS`, or one whose date cannot be read, is regenerated — and a legacy cert in
 * that state is not adopted. Renewal re-prompts for trust, but an expired cert already fails.
 *
 * Pure, with `exists` and `validUntil` injected, because the alternative is a function that can
 * only be tested by writing certificates into somebody's home directory.
 */
...
/**
 * Which port the HTTPS listener binds — decided AFTER the HTTP bind, because it depends on it.
 *
 * The well-known 8701 is the shared channel's https side: it is hardcoded in the injected loader,
 * in adopters' dev servers and in the user's per-origin cert trust. So it belongs to whichever
 * server holds the well-known HTTP port 8700, and to nobody else. Before this, every non-private
 * server reached for 8701: a `--port`/`--name` project server, or an unnamed one bumped off a busy
 * 8700 onto an ephemeral port. With `both` as the default, the first 1.13 server to start in ANY
 * project (say, next to a 1.12.x http-only channel on 8700) captured every project's https pages,
 * while `hj` and the 8700 server saw 0 tabs — the #32 silent misroute, reached by the default.
 * First-to-boot-wins is a race, not a policy (1.13.0-beta.1 blast-radius review).
 *
 * - private: ephemeral (it reports its address; it never touches shared ports).
 * - an explicit HTTPS port (`--https-port`, DEV_CHANNEL_HTTPS_PORT): that port. A person chose it.
 * - https-only: 8701. With no HTTP listener this server IS the https channel.
 * - holds 8700: 8701.
 * - anything else: ephemeral. HTTPS still comes up, at an address its banner and /status report,
 *   without claiming a port other projects' pages dial.
 */
export function httpsPortFor(opts: {
  isPrivate: boolean
  explicitHttpsPort: number | null
  /** The HTTP port actually bound, or null when HTTP is off. */
  boundHttpPort: number | null
}): number {
  if (opts.isPrivate) return 0
  if (opts.explicitHttpsPort !== null) return opts.explicitHttpsPort
  if (opts.boundHttpPort === null) return DEFAULT_HTTPS_PORT
  return opts.boundHttpPort === DEFAULT_HTTP_PORT ? DEFAULT_HTTPS_PORT : 0
}
