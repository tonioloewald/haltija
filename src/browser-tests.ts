 ...
 * 
 * These tests run inside the browser via the tosijs-dev widget.
 * They test the component's functionality from the browser's perspective.
 * 
 * Usage:
 * 1. Inject the tosijs-dev widget into any page
...
class BrowserTestRunner {
  ...
  async run(): Promise<TestSuite> {
    ...
    console.log('%c[tosijs-dev tests] Starting...', 'color: #6366f1; font-weight: bold')
    ...
    console.log(
      `%c[tosijs-dev tests] ${suite.passed}/${results.length} passed (${suite.duration.toFixed(1)}ms)`,
...
export function createBrowserTests(): BrowserTestRunner {
  ...
  runner.test('tosijs-dev element exists', () => {
    const el = document.querySelector('tosijs-dev')
    assertExists(el, 'tosijs-dev element should be in DOM')
  })
  
  runner.test('component has shadow root', () => {
    const el = document.querySelector('tosijs-dev') as HTMLElement
  ...
  runner.test('DOM query returns element info', async () => {
    ...
    testEl.id = 'tosijs-dev-test-element'
    ...
    try {
      ...
      if (el?.handleDomQuery) {
        const result = el.handleDomQuery({ selector: '#tosijs-dev-test-element' })
        assertExists(result, 'Query should return result')
        assertEqual(result.id, 'tosijs-dev-test-element')
  ...
  runner.test('DOM queryAll returns multiple elements', async () => {
    ...
    container.innerHTML = `
      <span class="tosijs-dev-multi-test">One</span>
      <span class="tosijs-dev-multi-test">Two</span>
      <span class="tosijs-dev-multi-test">Three</span>
    `
    ...
    try {
      ...
      if (el?.handleDomQuery) {
        const result = el.handleDomQuery({ 
          selector: '.tosijs-dev-multi-test',
          all: true 
        })
  ...
  runner.test('click dispatches event', async () => {
    ...
    testEl.id = 'tosijs-dev-click-test'
    document.body.appendChild(testEl)
    ...
    try {
      ...
      if (el?.handleSyntheticEvent) {
        await el.handleSyntheticEvent({
          selector: '#tosijs-dev-click-test',
          event: 'click'
        })
  ...
  runner.test('type enters text into input', async () => {
    ...
    testEl.id = 'tosijs-dev-type-test'
    document.body.appendChild(testEl)
